...
| Comando | Descripción |
|--------|-------------|
| `git -v`| ver la versión de Git que tenemos instalada |
| `git -h`| ver help, muestra los comandos más usados |
| `ls`| ver qué archivos / carpetas hay dentro de la carpeta donde nos encontramos |
| `cd <directorio>`| entra en un directorio concreto |
| `cd ..`| vuelve a un directorio atrás |
| `pwd`| ver la ruta actual en la que nos encontramos |
| `mkdir <nombre>`| añade un directorio (carpeta) en la ruta actual de nombre <nombre> |
| `touch <nombre.extensión>`| añade un archivo con el nombre <nombre> y extensión <.extensión> en la ruta actual |
| `rm <nombre>`| remueve un archivo existente en la ruta actual |
| `cp`| copia un archivo existente en la ruta actual |
| `mv`| mueve un archivo |
...
| `git init`| inicializar un repositorio en nuestro proyecto |
| `git status`| ver el estado de la rama actual y de Git ennuestro proyecto |
| `git add <archivo>`| añadir al área de Stage un archivo concreto |
| `git add .`| añadir al área de Stage todos los archivos pendientes de versionar |
| `git commit -m "<mensaje>"`| grabar una fotografía de todo lo que tenemos en el área de Stage |
| `git log`| historial de commits realizados (--graph / --decorate / --pretty=oneline ...) |
| `git checkout <archivo>`| volver al estado previo de un archivo antes de modificarlo (último commit del archivo). Permite moverse entre ramas o restaurar archivos |
| `git reset`| volver a la última fotografía (commit) completa tomada |
| ``|  |
| ``|  |
| ``|  |
| ``|  |
...
--> (En Markdown, para forzar salto de línea debes poner dos espacios "  " al final)
