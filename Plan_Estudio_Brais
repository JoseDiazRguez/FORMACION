* Aprender se aprende programando.

- Cada concepto --> Aplicarlo desde el principio 
    ==> Plantearme mis propios ejercicios (le pediré a la IA que me genere nuevas explicaciones y ejercicios para resolver)
- Cada programa y error que resuelvo --> Sirve para coger confianza 
    ==> Oportunidad para aprender a depurar errores.

- Aprender usando:
  - Documentación oficial, combinada con
  - Tutoriales o cursos  --> Universidad
  - Blog y Comunidades   --> Discord Mouredev

- Estar actualizados:
  - Suscripción a 1-2 Newsletters sobre lo que estamos estudiando y:
    --> Brais Moure
    --> https://www.codemotion.com/
  - Seguir referentes
    --> Brais Moure (@mouredev, Software engineer & Full-stack developer), 
    --> Miguel Ángel Durán (@midudev, Divulgador de programación), 
    --> Carlos Azaustre (@carlosazaustre, Software engineer), 
    --> Carlos Santana Vega (@DotCSV, IA)
    --> Chema Alonso (@chemaalonso, Ciberseguridad)
  - Crear espacio en GitHub paa organizar las novedades interesantes --> Registro de recursos útiles
